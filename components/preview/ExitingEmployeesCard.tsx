...
import { generateExitingBlurb } from './cardContent';
import type { Employee } from '@/types/newsletter';
import { EmployeeCard } from './EmployeeCard';

interface ExitingEmployeesCardProps {
  employees: Employee[];
}

export function ExitingEmployeesCard({ employees }: ExitingEmployeesCardProps) {
  const config = CARD_CONFIG.exitingEmployees;
  if (!employees || employees.length === 0) return null;

  if (employees.length === 1) {
    return <EmployeeCard employee={employees[0]} type="exitingEmployees" />;
  }
  ...
  return (
    <div className={`group relative overflow-hidden rounded-[2.5rem] border-2 ${config.theme} transition-all duration-500 hover:shadow-2xl hover:-translate-y-2`}>
      ...
      <div className="p-6 md:p-8">
        <h3 className="font-serif text-2xl font-extrabold leading-tight text-slate-900 mb-4">Farewells</h3>
          ...
          {employees.map(emp => {
            ...
            <div key={emp.id} className="flex items-center gap-4 rounded-2xl bg-white p-3 shadow-sm border border-slate-100">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-slate-700">{emp.position ? `${emp.position}` : 'Role not provided'}{emp.department ? ` in ${emp.department}.` : '.'}{emp.date ? ` Last day: ${emp.date}.` : ''}</p>
              </div>
