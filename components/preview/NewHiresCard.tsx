...
import { generateNewHiresBlurb } from './cardContent';
...
export function NewHiresCard({ employees }: NewHiresCardProps) {
  const config = CARD_CONFIG.newHires;
  if (!employees || employees.length === 0) return null;

  // single hires reuse EmployeeCard for full detail
  if (employees.length === 1) {
    return <EmployeeCard employee={employees[0]} type="newHires" />;
  }
  ...
  return (
    <div className={`group relative overflow-hidden rounded-[2.5rem] border-2 ${config.theme} transition-all duration-500 hover:shadow-2xl hover:-translate-y-2`}>
      ...
      <div className="p-6 md:p-8">
        <h3 className="font-serif text-2xl font-extrabold leading-tight text-slate-900 mb-4">New Hires</h3>
