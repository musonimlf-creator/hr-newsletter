...
export const RecentPostCard: React.FC<RecentPostCardProps> = ({ type, title, subtitle, caption, date, badge, avatarText, onClick }) => {
  ...
  return (
    <div
      ...
      className="rounded-2xl shadow-xl border-2 p-5 flex items-start gap-5 hover:scale-[1.02] transition cursor-pointer mb-4"
      tabIndex={0}
      onClick={onClick}
    >
      <div style={{ background: accent }} className="rounded-full w-14 h-14 flex items-center justify-center text-white font-extrabold text-2xl shrink-0 select-none">
      ...
      <div className="flex-1">
        <div className="flex gap-2 items-center mb-1">
          ...
          {badge && <span className="ml-2 bg-white text-xs px-3 py-1 rounded-full font-bold shadow-sm" style={{ color: accent }}>{badge}</span>}
        </div>
        ...
        <div className="flex gap-2 items-center mt-1">
          <span className="text-xs font-bold" style={{ color: accent }}>{cardTypeLabels[type]}</span>
